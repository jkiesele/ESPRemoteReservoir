...
// Displays reservoir data on a web page.
// Format:
// [Name] [connection LED] [level bar] X% | filled_l / capacity_l | temperature_C
...
public:
    ...
    String routeText() const override {
        String json;
        json.reserve(id_.length() + name_.length() + 190);
        ...
        const float filled = capacity * level * 0.01f;
    ...
    String createHtmlFragment() const override {
        ...
        html += "<span id='"; html += id(); html += "_vol'></span>";
        ...
        html += "<script>\n"
                ...
                "  const volEl=document.getElementById('"; html += id(); html += "_vol');\n"
                ...
                "      const filled = Math.max(0, Number(v.filled || 0));\n"
                ...
                "      volEl.textContent = ' | ' + filled.toFixed(1) + ' l / ' + capacity.toFixed(1) + ' l';\n"
    ...
    void loop() {
        // no need, the display is updated via polling in the client browser
    }
